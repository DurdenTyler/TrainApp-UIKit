...
class ProfileElements: UIView {
    ...
    private let text_Name = UILabel(text: "Иван Смит", fontName: "Roboto-Medium", fontSize: 26, textColor: .white, opacity: 1)
    
    private let text_Height = UILabel(text: "Рост: 190 см", fontName: "Roboto-Medium", fontSize: 18, textColor: .specialDarkBlue, opacity: 1)
    
    private let text_Weight = UILabel(text: "Вес: 90 кг", fontName: "Roboto-Medium", fontSize: 18, textColor: .specialDarkBlue, opacity: 1)
...
extension ProfileElements {
    private func setConstrains() {
        ...
        NSLayoutConstraint.activate([
            text_Height.topAnchor.constraint(equalTo: blueBlock.bottomAnchor, constant: 10),
            text_Height.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: 35)
        ])
        
        NSLayoutConstraint.activate([
            text_Weight.topAnchor.constraint(equalTo: blueBlock.bottomAnchor, constant: 10),
            text_Weight.leadingAnchor.constraint(equalTo: text_Height.trailingAnchor, constant: 10)
        ])
        
        NSLayoutConstraint.activate([
            button_Editing.topAnchor.constraint(equalTo: blueBlock.bottomAnchor, constant: 10),
